...
async def start(event):
    ...
    buttons = [
        [Button.inline('Input Data', b'input_data')],
        [Button.inline('Peroleh Lokasi Item', b'peroleh_lokasi')],
        [Button.inline('Peroleh Daftar Item Terdekat', b'peroleh_daftar')],
        [Button.inline('Peroleh File Set', b'peroleh_file')]
    ]
    text = 'Mohon pilih salah satu dari menu berikut.'
    await event.respond(text, buttons=buttons)
    raise events.StopPropagation

@client.on(events.CallbackQuery)
async def button_callback(event):
    button_pressed = event.data

    if button_pressed == b'input_data':
        buttons = [
            [Button.inline('PLACEHOLDER', b'button1')],
            [Button.inline('PLACEHOLDER', b'button2')]
        ]
        text = 'Mohon pilih salah satu dari metode input data berikut.'
        await event.answer(text, buttons=buttons)

    elif button_pressed == b'peroleh_lokasi':
        await event.respond('Mohon masukkan nama item.')
        raise events.StopPropagation

    elif button_pressed == b'peroleh_daftar':
        await event.answer('Mohon kirimkan lokasi.')

    elif button_pressed == b'peroleh_file':
        await event.answer('Mohon masukkan nama file yang mau diperoleh.')

    elif button_pressed == b'button1':
        await event.answer('You pressed the First Button!')

    elif button_pressed == b'button2':
        await event.answer('You pressed the Second Button!')

    await event.delete()

### Peroleh lokasi command
@client.on(events.NewMessage(pattern="(?i)/getloc"))
async def peroleh_lokasi(response):
